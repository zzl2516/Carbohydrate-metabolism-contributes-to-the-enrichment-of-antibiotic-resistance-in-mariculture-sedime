...
env <- read.csv("COG.txt",header = TRUE,sep = "\t",row.names = 1)
#载入第二个分析数据
env <- t(env)
env <- as.matrix(env)
#调整数据格式
...
tiff(filename = "ARGs_COG.tif",width = 5400,height = 5400,res = 600,type = "windows",compression = "lzw")
...
text(-2,1.5,labels = "r = 0.757, p-value = 0.0001",
